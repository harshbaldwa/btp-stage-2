...
@annotate(power='int', double='real, imag, multi_real, multi_imag, multi_factor', new_num='doublep')
def complex_power_multi(real, imag, power, multi_real, multi_imag, multi_factor, new_num):
    mag, ang = declare('double', 2)
    mag = (sqrt(real**2 + imag**2))**power
    ang = atan2(imag, real) * power
    new_num[0] = multi_factor * (mag*(cos(ang)) * multi_real - mag*(sin(ang)) * multi_imag)
    new_num[1] = multi_factor * (mag*(sin(ang)) * multi_real + mag*(cos(ang)) * multi_imag)
...
@annotate(int='i, p', intp='start, bin_count, indices', doublep='multipole_real, multipole_imag, prop_part, x_part, y_part, box_x, box_y')
def mulitpole_finest(i, start, bin_count, indices, multipole_real, multipole_imag, prop_part, x_part, y_part, box_x, box_y, p):
    ...
    for j in range(bin_count[i]):
        ...
        for k in range(1, p+1):
            complex_power(x_part[pid]-box_x[i], y_part[pid]-box_y[i], k, -prop_part[pid]/k, complex_num)
            multipole_real[box_number+k] += complex_num[0]
            multipole_imag[box_number+k] += complex_num[1]


@annotate(int='i, p', doublep='multipole_p_real, multipole_p_imag, multipole_c_real, multipole_c_imag, px, py, cx, cy')
def transfer_multipole(i, multipole_p_real, multipole_p_imag, multipole_c_real, multipole_c_imag, px, py, cx, cy, p):
    ...
    for j in range(4):
        ...
        if l == 0:
            ...
            for k in range(1, l+1):
                ...
                complex_power_multi(cx[center_c_id] - px[center_p_id], cy[center_c_id] - py[center_p_id], l-k, multipole_c_real[c_id+k], multipole_c_imag[c_id+k], mag_multi, complex_num)
                multipole_p_real[i] += complex_num[0]
                multipole_p_imag[i] += complex_num[1]
            complex_power(cx[center_c_id] - px[center_p_id], cy[center_c_id] - py[center_p_id], l, multipole_c_real[c_id]/l, complex_num)
            multipole_p_real[i] -= complex_num[0]
            multipole_p_imag[i] -= complex_num[1]


@annotate(int='cid, cell_level, pid, level, p', intp='bin_count, start, indices', doublep='prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, value', length='double')
def compute_cell_value(cid, cell_level, pid, value, prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, level, length, p, bin_count, start, indices):
    i, j = declare('int', 2)
    complex_num = declare('matrix(2)')
    if sqrt((cx[cid] - x_part[pid])**2 + (cy[cid] - y_part[pid])**2) > length/(2**(cell_level-1)):
        value[pid] += multipole_real[cid*(p+1)]*log_complex(x_part[pid] - cx[cid], y_part[pid] - cy[cid])
        for j in range(1, p+1):
            complex_power_multi(x_part[pid]-cx[cid], y_part[pid]-cy[cid], -j, multipole_real[cid*(p+1)+j], multipole_imag[cid*(p+1)+j], 1, complex_num)
            value[pid] += complex_num[0]
    elif cell_level < level:
        for i in range(4):
            compute_cell_value(4*(cid-4**level)+i, cell_level+1, pid, value, prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, level, length, p, bin_count, start, indices)
    else:
        for i in range(bin_count[cid]):
            if pid != indices[start[cid] + i]:
                value[pid] += prop_part[indices[start[cid] + i]]*log_complex(x_part[pid] - x_part[indices[start[cid] + i]], y_part[pid] - y_part[indices[start[cid] + i]])


@annotate(int='i, level, p, total_blocks', intp='bin_count, start, indices', doublep='prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, value', length='double')
def compute_value(i, value, prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, level, length, p, bin_count, start, indices, total_blocks):
    j = declare('int')
    for j in range(16):
        compute_cell_value(total_blocks-16+j, 2, i, value, prop_part, x_part, y_part, multipole_real, multipole_imag, cx, cy, level, length, p, bin_count, start, indices)
...
wasteblocks = 4**(level+1) * int((4**(10-level) - 1)/3)
...
x, y, bin_count, start, bin_offset, indices, cx, cy, multipole_real, multipole_imag, value = wrap(x, y, bin_count, start, bin_offset, indices, cx, cy, multipole_real, multipole_imag, value, backend=backend)
...
emultipole_finest(start, bin_count, indices, multipole_real, multipole_imag, prop_part, x, y, cx, cy, p)
for l in range(level, 2, -1):
    index1 = int((4**(level-l) - 1)/3)*4**(l+1)
    index2 = int((4**(level-l+1) - 1)/3)*4**l
    index3 = int((4**(level-l+2) - 1)/3)*4**(l-1)
    etransfer_multipole(multipole_real[index2*(p+1):index3*(p+1)], multipole_imag[index2*(p+1):index3*(p+1)], multipole_real[index1*(p+1):index2*(p+1)], multipole_imag[index1*(p+1):index2*(p+1)], cx[index2:index3], cy[index2:index3], cx[index1:index2], cy[index1:index2], p)

ecompute_value(value, prop_part, x, y, multipole_real, multipole_imag, cx, cy, level, length, p, bin_count, start, indices, total_blocks)
...
#     for j in range(n):
#         if i != j:
#             value_real[i] += prop_part_direct[j]*np.log(abs((x_direct[i]-x_direct[j]) + 1j*(y_direct[i]-y_direct[j])))
...
# print("Speedup - ", (time_stop_direct-time_start_direct)/(time_stop_fmm-time_start_fmm))
